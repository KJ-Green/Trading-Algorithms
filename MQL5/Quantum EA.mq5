...
input int         InpRecoveryTimes     = 3;           //Recovery Times Multiplier  
input bool        InpSmartRecovery     = true;        //Use Smart Recovery Mode  
...
double      orgnl_openPrice, sellRec_threshMax, sellRec_threshMid, sellRec_threshLow, buyRec_threshMax, buyRec_threshMid, buyRec_threshLow;
int         smartRec_SellCount, smartRec_BuyCount;
...
//---
   ...
   if(ordSell==0 && posSell==0){sellTS_level=0; orgnl_openPrice=0;}

   if(posBuy==0){dd_buysRecovered=0; smartRec_BuyCount=0;}
   if(posSell==0){dd_sellsRecovered=0; smartRec_SellCount=0;}
   
   if(in_drawdown==true){
      sellRec_threshMax = orgnl_openPrice+1250*_Point;
      sellRec_threshMid = orgnl_openPrice+750*_Point;
      sellRec_threshLow = orgnl_openPrice+200*_Point;
      
      buyRec_threshMax = orgnl_openPrice-1250*_Point;
      buyRec_threshMid = orgnl_openPrice-750*_Point;
      buyRec_threshLow = orgnl_openPrice-200*_Point;      
   }
...
//auto lot (based on 100k balance | 7 orders)
  ...
  if(in_drawdown==true){original_lotSize=NormalizeDouble(lotSize,2); lotSize = NormalizeDouble((lotSize*InpRecoveryMulti),2);}
...
//=== CHECK FOR FAILED TRAILING STOP IN DRAWDOWN ====================================================//

   if(posSell>0 && open_recSells==0 && dd_sellsRecovered>0){   
      for(int i = PositionsTotal()-1; i>=0; i--){
         ...
         if(symbol==_Symbol && type==POSITION_TYPE_SELL && volume<lotSize && currentTick.bid<=sellTS_level && pos_SL != sellTS_level+InpTrailingStep*_Point){
   ...
   if(posBuy>0 && open_recBuys==0 && dd_buysRecovered>0){   
      for(int i = PositionsTotal()-1; i>=0; i--){
         string symbol = PositionGetSymbol(i);
         double volume = PositionGetDouble(POSITION_VOLUME);
         ulong positionTicket = PositionGetInteger(POSITION_TICKET);
         ...
         if(symbol==_Symbol && type==POSITION_TYPE_BUY && volume<lotSize && currentTick.ask>=buyTS_level && pos_SL != buyTS_level-InpTrailingStep*_Point){
            trade.PositionClose(positionTicket,-1);
         }
...
//=== FORCE CLOSE RECOVERY POSITIONS IF THEY ARE STILL OPEN WHEN ORIGINAL POSITIONS CLOSE ========================//   

   if(in_drawdown==true && open_orgnlBuys==0 && open_orgnlSells==0 && (open_recBuys>0 || open_recSells>0)){
      for(int i = PositionsTotal()-1; i>=0; i--){
         string symbol = PositionGetSymbol(i);
         ulong positionTicket = PositionGetInteger(POSITION_TICKET);
         long type = PositionGetInteger(POSITION_TYPE);
...
//   if(in_drawdown==true && open_orgnlSells==0 && (open_recBuys>0 || open_recSells>0)){
//      for(int i = PositionsTotal()-1; i>=0; i--){
//         string symbol = PositionGetSymbol(i);
//         ulong positionTicket = PositionGetInteger(POSITION_TICKET);
//         long type = PositionGetInteger(POSITION_TYPE);
...
//
//
//         if(symbol==_Symbol  && type==POSITION_TYPE_SELL && magic==InpMagicNumber){
//            trade.PositionClose(positionTicket,-1);
//         }
//  
//      }         
...
//=== CHECKING FOR OPEN RECOVERY POSITIONS ==============================================//
   ...
   //Comment("oegnl_openprice: ",orgnl_openPrice);
...
//      if(dd_sellsRecovered==1 && open_orgnlSells>0){  
//         for(int i = PositionsTotal()-1; i>=0; i--){
//            string symbol = PositionGetSymbol(i);
//            ulong positionTicket = PositionGetInteger(POSITION_TICKET);
//            long type = PositionGetInteger(POSITION_TYPE);
//            long magic = PositionGetInteger(POSITION_MAGIC); 
//            double posVolume = PositionGetDouble(POSITION_VOLUME);
//            if(symbol==_Symbol && type==POSITION_TYPE_SELL && magic==InpMagicNumber && dd_sellsRecovered==1){ 
//               trade.PositionClose(positionTicket,-1);
//               dd_sellsRecovered=2;
//            }                       
//         }
...
//   }
//   
//   if((in_drawdown==true && recovery_buysClose>13 && recovery_buysClose<23) || (in_drawdown==true && recovery_sellsClose>13 && recovery_sellsClose<23)){         
//      if(dd_sellsRecovered==2 && open_orgnlSells>0){  
//         for(int i = PositionsTotal()-1; i>=0; i--){
//            string symbol = PositionGetSymbol(i);
//            ulong positionTicket = PositionGetInteger(POSITION_TICKET);
//            long type = PositionGetInteger(POSITION_TYPE);
//            long magic = PositionGetInteger(POSITION_MAGIC); 
//            double posVolume = PositionGetDouble(POSITION_VOLUME);
//            if(symbol==_Symbol && type==POSITION_TYPE_SELL && magic==InpMagicNumber && dd_sellsRecovered==2){ 
//               trade.PositionClose(positionTicket,-1);
//               dd_sellsRecovered=3;
//            }                       
//         }
...
//   }
//
//   if((in_drawdown==true && recovery_buysClose>17 && recovery_buysClose<31) || (in_drawdown==true && recovery_sellsClose>17 && recovery_sellsClose<31)){         
//      if(dd_sellsRecovered==3 && open_orgnlSells>0){   
//         for(int i = PositionsTotal()-1; i>=0; i--){
//            string symbol = PositionGetSymbol(i);
//            ulong positionTicket = PositionGetInteger(POSITION_TICKET);
//            long type = PositionGetInteger(POSITION_TYPE);
//            long magic = PositionGetInteger(POSITION_MAGIC); 
//            double posVolume = PositionGetDouble(POSITION_VOLUME);
//            if(symbol==_Symbol && type==POSITION_TYPE_SELL && magic==InpMagicNumber && dd_sellsRecovered==3){ 
//               trade.PositionClose(positionTicket,-1);
//               dd_sellsRecovered=4;
//            }                       
//         }
...
//   }   
//   
//   if((in_drawdown==true && recovery_buysClose>20 && recovery_buysClose<39) || (in_drawdown==true && recovery_sellsClose>20 && recovery_sellsClose<39)){         
//      if(dd_sellsRecovered==4 && open_orgnlSells>0){   
//         for(int i = PositionsTotal()-1; i>=0; i--){
//            string symbol = PositionGetSymbol(i);
//            ulong positionTicket = PositionGetInteger(POSITION_TICKET);
//            long type = PositionGetInteger(POSITION_TYPE);
//            long magic = PositionGetInteger(POSITION_MAGIC); 
//            double posVolume = PositionGetDouble(POSITION_VOLUME);
//            if(symbol==_Symbol && type==POSITION_TYPE_SELL && magic==InpMagicNumber && dd_sellsRecovered==4){ 
//               trade.PositionClose(positionTicket,-1);
//               dd_sellsRecovered=5;
//            }                       
//         }
...
//   }
//
//   if((in_drawdown==true && recovery_buysClose>26 && recovery_buysClose<47) || (in_drawdown==true && recovery_sellsClose>26 && recovery_sellsClose<47)){         
//      if(dd_sellsRecovered==5 && open_orgnlSells>0){  
//         for(int i = PositionsTotal()-1; i>=0; i--){
//            string symbol = PositionGetSymbol(i);
//            ulong positionTicket = PositionGetInteger(POSITION_TICKET);
//            long type = PositionGetInteger(POSITION_TYPE);
//            long magic = PositionGetInteger(POSITION_MAGIC); 
//            double posVolume = PositionGetDouble(POSITION_VOLUME);
//            if(symbol==_Symbol && type==POSITION_TYPE_SELL && magic==InpMagicNumber && dd_sellsRecovered==5){ 
//               trade.PositionClose(positionTicket,-1);
//               dd_sellsRecovered=6;
//            }                       
//         }
...
//   }
//
//   if((in_drawdown==true && recovery_buysClose>32 && recovery_buysClose<55) || (in_drawdown==true && recovery_sellsClose>32 && recovery_sellsClose<55)){         
//      if(dd_sellsRecovered==6 && open_orgnlSells>0){ 
//         for(int i = PositionsTotal()-1; i>=0; i--){
//            string symbol = PositionGetSymbol(i);
//            ulong positionTicket = PositionGetInteger(POSITION_TICKET);
//            long type = PositionGetInteger(POSITION_TYPE);
//            long magic = PositionGetInteger(POSITION_MAGIC); 
//            double posVolume = PositionGetDouble(POSITION_VOLUME);
//            if(symbol==_Symbol && type==POSITION_TYPE_SELL && magic==InpMagicNumber && dd_sellsRecovered==6){ 
//               trade.PositionClose(positionTicket,-1);
//               dd_sellsRecovered=7;
//            }                       
//         }
...
//   if(InpSmartRecovery==true && in_drawdown==true){
//      for(int i = PositionsTotal()-1; i>=0; i--){
//         string symbol = PositionGetSymbol(i);
//         ulong positionTicket = PositionGetInteger(POSITION_TICKET);
//         long type = PositionGetInteger(POSITION_TYPE);
...
//         double posVolume = PositionGetDouble(POSITION_VOLUME);
////-SELL RECOVERY              
//         if(currentTick.bid<=sellRec_threshMax && currentTick.bid>sellRec_threshMid){
//            if(type==POSITION_TYPE_SELL && posVolume<lotSize && magic==InpMagicNumber){
//               trade.PositionClose(positionTicket,-1);
//               smartRec_SellCount=1;
//            }
//         }    
//      }              
//   } 
...
//=== RECOVERY MODE CLOSE BUYS =====================================================================================//   
   ...
   if((in_drawdown==true && recovery_buysClose==0 && recovery_sellsClose==7) || (in_drawdown==true && recovery_buysClose==7 && recovery_sellsClose==0)){
      ...
      for(int i = PositionsTotal()-1; i>=0; i--){
         ...
         double posVolume = PositionGetDouble(POSITION_VOLUME);
//---         
         //if(currentTick.bid >= buyRec_threshMax){
         if(dd_buysRecovered==0 && open_orgnlBuys>0 && symbol==_Symbol && type==POSITION_TYPE_BUY && magic==InpMagicNumber){
            trade.PositionClose(positionTicket,-1);
            dd_buysRecovered=1; 
         }
         //}
         //if(currentTick.bid <= sellRec_threshMax){    
         if(dd_sellsRecovered==0 && open_orgnlSells>0 && symbol==_Symbol && type==POSITION_TYPE_SELL && magic==InpMagicNumber){
            trade.PositionClose(positionTicket,-1);
            dd_sellsRecovered=1; 
         }   
         //}                                  
//---
//         if(currentTick.bid >= buyRec_threshMid){
//            if(dd_buysRecovered<2 && open_orgnlBuys>0 && symbol==_Symbol && type==POSITION_TYPE_BUY && magic==InpMagicNumber){
//               trade.PositionClose(positionTicket,-1);
//               dd_buysRecovered=2; 
//            }
//         }
//         if(currentTick.bid <= sellRec_threshMid){    
//            if(dd_sellsRecovered<2 && open_orgnlSells>0 && symbol==_Symbol && type==POSITION_TYPE_SELL && magic==InpMagicNumber){
//               trade.PositionClose(positionTicket,-1);
//               dd_sellsRecovered=2; 
//            }   
//         } 
////---
//         if(currentTick.bid >= buyRec_threshLow){
//            if(dd_buysRecovered<3 && open_orgnlBuys>0 && symbol==_Symbol && type==POSITION_TYPE_BUY && magic==InpMagicNumber){
//               trade.PositionClose(positionTicket,-1);
//               dd_buysRecovered=3; 
//            }
//         }
//         if(currentTick.bid <= sellRec_threshLow){    
//            if(dd_sellsRecovered<3 && open_orgnlSells>0 && symbol==_Symbol && type==POSITION_TYPE_SELL && magic==InpMagicNumber){
//               trade.PositionClose(positionTicket,-1);
//               dd_sellsRecovered=3; 
//            }   
//         }          
   ...
   if((in_drawdown==true && recovery_buysClose==7 && recovery_sellsClose==7) || (in_drawdown==true && recovery_buysClose==14 && recovery_sellsClose==0) ||
      (in_drawdown==true && recovery_buysClose==0 && recovery_sellsClose==14)){         
...
//---
         //if(currentTick.bid >= buyRec_threshMax){         
         if(dd_buysRecovered==1 && open_orgnlBuys>0 && symbol==_Symbol && type==POSITION_TYPE_BUY && magic==InpMagicNumber){
            trade.PositionClose(positionTicket,-1);
            dd_buysRecovered=2;                                                                                                                
         }
         //}
         //if(currentTick.bid <= sellRec_threshMax){
         if(dd_sellsRecovered==1 && open_orgnlSells>0 && symbol==_Symbol && type==POSITION_TYPE_SELL && magic==InpMagicNumber){
            trade.PositionClose(positionTicket,-1);
            dd_sellsRecovered=2;                                                                                                                
         }                                        
         //}  
//---
//         if(currentTick.bid >= buyRec_threshMid){         
//            if(dd_buysRecovered<4 && open_orgnlBuys>0 && symbol==_Symbol && type==POSITION_TYPE_BUY && magic==InpMagicNumber){
//               trade.PositionClose(positionTicket,-1);
//               dd_buysRecovered=4;                                                                                                                
//            }
//         }
//         if(currentTick.bid <= sellRec_threshMid){
//            if(dd_sellsRecovered<4 && open_orgnlSells>0 && symbol==_Symbol && type==POSITION_TYPE_SELL && magic==InpMagicNumber){
//               trade.PositionClose(positionTicket,-1);
//               dd_sellsRecovered=4;                                                                                                                
//            }                                        
//         } 
////---
//         if(currentTick.bid >= buyRec_threshLow){         
//            if(dd_buysRecovered<6 && open_orgnlBuys>0 && symbol==_Symbol && type==POSITION_TYPE_BUY && magic==InpMagicNumber){
//               trade.PositionClose(positionTicket,-1);
//               dd_buysRecovered=6;                                                                                                                
//            }
//         }
//         if(currentTick.bid <= sellRec_threshLow){
//            if(dd_sellsRecovered<6 && open_orgnlSells>0 && symbol==_Symbol && type==POSITION_TYPE_SELL && magic==InpMagicNumber){
//               trade.PositionClose(positionTicket,-1);
//               dd_sellsRecovered=6;                                                                                                                
//            }                                        
//         }                                                                                              
   ...
   if((in_drawdown==true && recovery_buysClose==14 && recovery_sellsClose==7) || (in_drawdown==true && recovery_buysClose==7 && recovery_sellsClose==14) ||
      (in_drawdown==true && recovery_buysClose==0 && recovery_sellsClose==21) || (in_drawdown==true && recovery_buysClose==21 && recovery_sellsClose==0)){         
      ...
      for(int i = PositionsTotal()-1; i>=0; i--){
         ...
         //if(InpRecoveryTimes==2){
         //   if(symbol==_Symbol && magic==InpMagicNumber && (type==POSITION_TYPE_BUY || type==POSITION_TYPE_SELL)){
         //      trade.PositionClose(positionTicket,-1);
         //   }   
         //}
   ...
   if((in_drawdown==true && recovery_buysClose==14 && recovery_sellsClose==14) || (in_drawdown==true && recovery_buysClose==21 && recovery_sellsClose==7) ||
      (in_drawdown==true && recovery_buysClose==7 && recovery_sellsClose==21) || (in_drawdown==true && recovery_buysClose==0 && recovery_sellsClose==28) ||
      (in_drawdown==true && recovery_buysClose==28 && recovery_sellsClose==0)){         
   ...
   if((in_drawdown==true && recovery_buysClose==14 && recovery_sellsClose==21) || (in_drawdown==true && recovery_buysClose==21 && recovery_sellsClose==14) ||
      (in_drawdown==true && recovery_buysClose==28 && recovery_sellsClose==7) || (in_drawdown==true && recovery_buysClose==7 && recovery_sellsClose==28) ||
      (in_drawdown==true && recovery_buysClose==0 && recovery_sellsClose==35) || (in_drawdown==true && recovery_buysClose==35 && recovery_sellsClose==0)){         
   ...
   if((in_drawdown==true && recovery_buysClose==21 && recovery_sellsClose==21) || (in_drawdown==true && recovery_buysClose==28 && recovery_sellsClose==14) ||
      (in_drawdown==true && recovery_buysClose==14 && recovery_sellsClose==28) || (in_drawdown==true && recovery_buysClose==35 && recovery_sellsClose==7) ||
      (in_drawdown==true && recovery_buysClose==7 && recovery_sellsClose==35) || (in_drawdown==true && recovery_buysClose==0 && recovery_sellsClose==42) ||
      (in_drawdown==true && recovery_buysClose==42 && recovery_sellsClose==0)){         
      ...
      for(int i = PositionsTotal()-1; i>=0; i--){
         ...
         if(InpRecoveryTimes==5){
            if(symbol==_Symbol && magic==InpMagicNumber && (type==POSITION_TYPE_BUY || type==POSITION_TYPE_SELL)){
               trade.PositionClose(positionTicket,-1);
            }   
         }           
         
         if(dd_buysRecovered==5 && open_orgnlBuys>0 && symbol==_Symbol && type==POSITION_TYPE_BUY && magic==InpMagicNumber){ 
            trade.PositionClose(positionTicket,-1);
            dd_buysRecovered=6;
         }
         if(dd_sellsRecovered==5 && open_orgnlSells>0 && symbol==_Symbol && type==POSITION_TYPE_SELL && magic==InpMagicNumber){ 
            trade.PositionClose(positionTicket,-1);
            dd_sellsRecovered=6;
         }                                 
      }
   ...
   if((in_drawdown==true && recovery_buysClose==28 && recovery_sellsClose==21) || (in_drawdown==true && recovery_buysClose==21 && recovery_sellsClose==28) ||
      (in_drawdown==true && recovery_buysClose==35 && recovery_sellsClose==14) || (in_drawdown==true && recovery_buysClose==14 && recovery_sellsClose==35) ||
      (in_drawdown==true && recovery_buysClose==42 && recovery_sellsClose==7) || (in_drawdown==true && recovery_buysClose==7 && recovery_sellsClose==42) ||
      (in_drawdown==true && recovery_buysClose==0 && recovery_sellsClose==49) || (in_drawdown==true && recovery_buysClose==49 && recovery_sellsClose==0)){         
...
void  checkTrailingSLBuy(double ask){
   ...
   for(int i=PositionsTotal()-1; i>=0; i--){
      ...
      if(_Symbol==symbol){
         ...
         if(in_drawdown==true && type==POSITION_TYPE_BUY && volume>original_lotSize && magic==InpMagicNumber){
            buyTS_level = openPrice+InpTrailingStart*_Point;
         } 
         ...
         //}
         if(in_drawdown==true){    
            if(magic==InpMagicNumber && type==POSITION_TYPE_BUY && currentTick.bid>=buyTS_level && volume>original_lotSize){ // && volume==lotSize
               trade.PositionModify(positionTicket,buyTS,buyTS_TP);
            }
...
void  checkTrailingSLSell(double bid){
   ...
   for(int i=PositionsTotal()-1; i>=0; i--){
      ...
      if(_Symbol==symbol){
         ...
         if(in_drawdown==true && open_recSells==0 && ordSell==0){
            if(magic==InpMagicNumber && volume<lotSize && type==POSITION_TYPE_SELL){
               if(currentTick.bid>openPrice){
                  sellTS_level=openPrice-InpTrailingStart*_Point;
               }
               if(currentTick.bid<= sellTS_level){
                  trade.PositionModify(positionTicket,sellTS,sellTS_TP);
                                                           ...
                                                           //volume>original_lotSize
         if(in_drawdown==true && type==POSITION_TYPE_SELL && volume>original_lotSize && magic==InpMagicNumber){
            sellTS_level = openPrice-InpTrailingStart*_Point;
         }  
         ...
         if(in_drawdown==true){    
            if(magic==InpMagicNumber && type==POSITION_TYPE_SELL && currentTick.bid<=sellTS_level && volume>original_lotSize){ // && volume==lotSize
               trade.PositionModify(positionTicket,sellTS,sellTS_TP);
            }
            if(currentTick.bid<=sellTS_level){
               sellTS_level = sellTS_level-InpTrailingStep*_Point;
            }            
         }  
...
//---if transaction is result of addition TO the transaction in history
   if(type==TRADE_TRANSACTION_DEAL_ADD){
      ...
      if(in_drawdown==true){
         //detecting recovery sells exit (to close one buy position in dd)                      //&& deal_volume==lotSize
         if(deal_entry==DEAL_ENTRY_OUT && deal_type==DEAL_TYPE_BUY && deal_magic==InpMagicNumber && deal_volume>original_lotSize){
            //recovery_sellsClose = 1;
            recovery_sellsClose++;
         }
         //detecting recovery buys exit (to close one sell position in dd)
         if(deal_entry==DEAL_ENTRY_OUT && deal_type==DEAL_TYPE_SELL && deal_magic==InpMagicNumber && deal_volume>original_lotSize){
            //recovery_buysClose = 1;
            recovery_buysClose++;
         }
         if(deal_entry==DEAL_ENTRY_IN && deal_volume<lotSize && deal_magic==InpMagicNumber){
            orgnl_openPrice = deal_price;
         }
      }
